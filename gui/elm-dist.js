...
var $author$project$Main$modelEditFileDecoder = A3(
	$elm$json$Json$Decode$map2,
	$author$project$Main$EditFile,
	A2($elm$json$Json$Decode$field, 'filename', $elm$json$Json$Decode$string),
	A2($elm$json$Json$Decode$field, 'content', $elm$json$Json$Decode$string));
var $author$project$Main$chooseStateDecoder = function (state) {
	switch (state) {
		case 'Loading':
			return $elm$json$Json$Decode$succeed($author$project$Main$Loading);
		case 'EditFile':
			return $author$project$Main$modelEditFileDecoder;
		default:
			return $elm$json$Json$Decode$fail('Invalid state type: ' + state);
	}
};
var $author$project$Main$modelDecoder = A2(
	$elm$json$Json$Decode$andThen,
	$author$project$Main$chooseStateDecoder,
	A2($elm$json$Json$Decode$field, 'state', $elm$json$Json$Decode$string));
var $author$project$Main$decodeValue = function (x) {
	var result = A2($elm$json$Json$Decode$decodeValue, $author$project$Main$modelDecoder, x);
	if (!result.$) {
		var model = result.a;
		return $author$project$Main$ChangeState(model);
	} else {
		var err = result.a;
...
var $author$project$Main$toRustCmd = function (command) {
	var value = function () {
		if (!command.$) {
			var level = command.a;
			var msg = command.b;
	...
	return $author$project$Main$toRust(value);
};
	...
	function (msg, model) {
		if (msg.$ === 1) {
			...
			return _Utils_Tuple2(
				model,
				$author$project$Main$toRustCmd(command));
		} else {
			var newModel = msg.a;
										...
										_List_fromArray(
											[
												...
												$author$project$Main$SendToRust(
													A2($author$project$Main$UploadFile, filename, content)))
											]),
										_List_fromArray(
											[
